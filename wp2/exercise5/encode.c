...
/*
Because using the union struct combination, we never actually have to shift
ourselves, the compiler sort of does it for us Theoretically speaking, you can
simply input an integer in value and that will also set the correct bits Very
nifty thing!
*/
...
int main(int argc, char *argv[]) { // main functions, arguments enabled
  ...
  for (int j = 1; j < argc; j++) {
    ...
    // j is the different arguments, 1 engine_pos, 2 gear_pos, 3 key_pos, 4
    // front brake, 5 rear break
    switch (j) {
    case 1: // engine_pos
      if (atoi(argv[j]) < 0 ||
          atoi(argv[j]) > 1) { // checking if argument is valid input
    ...
    case 2: // gear_pos
      if (atoi(argv[j]) < 0 ||
          atoi(argv[j]) > 4) { // checking if argument is valid input
        printf("Invalid input\n");
        return 0;
      }
      break;
    case 3: // key_pos
      if (atoi(argv[j]) < 0 ||
          atoi(argv[j]) > 2) { // checking if argument is valid input
        printf("Invalid input\n");
        return 0;
      }
      break;
  ...
  BitMap map = {
      0}; // initialize map and set all values to 0 to get rid of garbage values

  map.bits.engine_on =
      atoi(argv[1]); // set value to argument with atoi (ASCII TO INT)
  if (atoi(argv[2]) < 0 || atoi(argv[2]) > 4) {
    printf("Invalid gear");
    return 0;
  }
  map.bits.gear_pos =
      atoi(argv[2]); // set value to argument with atoi (ASCII TO INT)
  map.bits.key_pos =
      atoi(argv[3]); // set value to argument with atoi (ASCII TO INT)
  map.bits.brake1 =
      atoi(argv[4]); // set value to argument with atoi (ASCII TO INT)
  map.bits.brake2 =
      atoi(argv[5]); // set value to argument with atoi (ASCII TO INT)
