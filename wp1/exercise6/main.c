...
int main(int argc, char **argv) {
  ...
  if (argc == 4 && strcmp("-horizontal", argv[3]) ==
                       0) { // check if the vertical flag is passed
    draw_histogram_horizontal(frequency); // draw the frequency as a horizontal histogram
  } else {
    draw_histogram_vertical(frequency); // draw the frequency as a vertical histogram
  }
}
...
void draw_histogram_horizontal(int *freq) {
  ...
  // print the histogram by printing the same number of rows as the maxFreq holds
  for (int row = 0; row < maxFreq; row++) {
    for (int col = 0; col < maxNumber; col++) {
      ...
      if (maxFreq - row <= freq[col]) {
        printf(" * ");
      } else { // if the value of maxFreq - row is greater than freq[col], just print whitespace
        printf("   ");
      }
    }
